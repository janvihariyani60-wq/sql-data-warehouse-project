...
Script Purpose:
    ...
    populate the Silver layer tables from the Bronze layer tables.
	
Actions Performed:
		- Truncates Silver tables.
		- Inserts transformed and cleansed data from Bronze layer tables into Silver layer tables.

Usage Example:
    Run this Script directly to refresh the Silver layer.
===============================================================================
*/
...
-- UPDATE THE DDL AFTER TRANSFORAMATIONS IF DATATYPE OF COLUMNS CHANGED OR NEW COLUMNS DERIVED DURING TRANSFORMATIONS. 
...
SELECT
    ...
    END AS sls_due_dt,
    CASE 
		WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(NULLIF(sls_price,0)) 
...
SELECT
    ...
    CASE 
		...
		ELSE bdate
	END AS bdate,  -- Set future birthdatesto NULL
    CASE 
